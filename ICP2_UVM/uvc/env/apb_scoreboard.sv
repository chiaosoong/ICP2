...
class apb_scoreboard extends uvm_scoreboard;
    ...
    // -----------------------------------------------------------------
    // flush: Clears all stored data in the scoreboard.
    // Useful when resetting the DUT in the middle of a test.
    // -----------------------------------------------------------------
    function void flush();
        drv_data.delete();
        ca_ifm_items.delete();
        ca_kernel_items.delete();
        exp_seq_item_q.delete();
        rcvd_seq_item_q.delete();
    ...
    // -----------------------------------------------------------------
    function void construct_nd_push_exp_pkt(input logic [`ADDR_WIDTH-1:0] ADDR, input logic [`DATA_WIDTH-1:0] DATA);
        apb_seq_item exp_seq_item;
        // construct seq_item
    ...
    function void write_mntr2scb(apb_seq_item item);
        // Capture READ transactions for comparison
        if (item.op_type == READ) begin
    ...
    function void compare_pkt(apb_seq_item exp_pkt, apb_seq_item rcvd_pkt);
        if (exp_pkt.ADDR != rcvd_pkt.ADDR) begin
            `uvm_error("SCB_CMP_ADDR", $sformatf("Address Mismatch! Exp=0x%0h Rcvd=0x%0h", exp_pkt.ADDR, rcvd_pkt.ADDR))
        end
        if (exp_pkt.DATA != rcvd_pkt.DATA) begin
            `uvm_error("SCB_CMP_DATA", $sformatf("Data Mismatch! Exp=0x%0h Rcvd=0x%0h", exp_pkt.DATA, rcvd_pkt.DATA))
        end else begin
