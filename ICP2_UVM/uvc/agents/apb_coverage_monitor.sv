class apb_coverage_monitor extends uvm_subscriber#(apb_seq_item);
    ...
    // APB Register Address Coverage
    covergroup apb_reg_addr_cg;
        REG_ADDR: coverpoint addr iff(addr_cg_valid) {
            bins data_reg    = {DATA_ADDR};       
            bins input_mode  = {INPUT_MODE_ADDR}; 
            bins kernel_mode = {KERNEL_MODE_ADDR};
            bins output_reg  = {OUTPUT_ADDR};      
            bins ca_finish   = {CA_FINISHED_ADDR};
            bins all_done    = {ALL_READ_DONE_ADDR};
            bins burst_mode  = {READ_CTRL_ADDR};
        }
    endgroup
    ...
    virtual function void write(apb_seq_item t);
        ...
        // Sample APB Register Address
        addr = t.ADDR;
        apb_reg_addr_cg.sample();
