class apb_sram_rd_after_reset extends apb_base_test;
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        if (apb_env.apb_mstr_agnt.apb_cov_mntr != null) begin
            ...
            apb_env.apb_mstr_agnt.apb_cov_mntr.addr_cg_valid = 1;
        end 
    ...
    task run_functional_check();
        ...
        // Check ALL_READ_DONE
        begin
            apb_rd_sequence rd_done_seq;
            rd_done_seq = apb_rd_sequence::type_id::create("rd_done_seq");
            rd_done_seq.addr = ALL_READ_DONE_ADDR;
            rd_done_seq.rand_addr = 0;
            rd_done_seq.start(apb_env.apb_mstr_agnt.apb_mstr_seqr);
            if (rd_done_seq.item.DATA[0] !== 1) begin
                `uvm_error("TEST", "ALL_READ_DONE not asserted after reading all outputs!")
            end else begin
                `uvm_info("TEST", "ALL_READ_DONE asserted correctly.", UVM_LOW)
            end
        end
